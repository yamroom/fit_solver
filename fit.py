...
class SympyModel:
    ...
    def parse(expr_str: str) -> "SympyModel":
        ...
        # Use together() to expose a rational structure before fraction()
    ...
    def df_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> np.ndarray:
        arr = np.asarray(self.df_dp_lam(x, i_val, *theta))
        # Ensure shape (P, N)
    ...
    def ddfdx_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> np.ndarray:
        arr = np.asarray(self.ddfdx_dp_lam(x, i_val, *theta))
        arr = np.atleast_2d(arr)
        if arr.ndim == 1:
            arr = arr[:, None]
        return arr.astype(float, copy=False)
    ...
    def d2fdx2_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> np.ndarray:
        arr = np.asarray(self.d2fdx2_dp_lam(x, i_val, *theta))
        arr = np.atleast_2d(arr)
        if arr.ndim == 1:
            arr = arr[:, None]
        return arr.astype(float, copy=False)
    ...
    def dden_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> Optional[np.ndarray]:
        if self.dden_dp_lam is None:
            return None
        arr = np.asarray(self.dden_dp_lam(x, i_val, *theta))
        arr = np.atleast_2d(arr)
        if arr.ndim == 1:
            arr = arr[:, None]
        return arr.astype(float, copy=False)
