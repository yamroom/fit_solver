...
class SolverConfig:
    ...
    # Relative error r_rel=(f-y)/max(|y|, rel_eps)
...
class FitGlobal:
    ...
    def _init_thetas_global(self) -> Dict[float, np.ndarray]:
        ...
        for g in self.groups:
            ...
            if xg.size >= max(2, self.P):
                ...
                try:
                    baseline = np.array([1.0 if n.startswith("C1_") else 0.0 for n in self.model.param_names], dtype=float)
                    res = least_squares(
                        resid, x0=baseline, jac=jac,
    ...
    def solve_least_squares(self) -> Tuple[np.ndarray, Dict]:
        """Least-squares path using scipy.optimize.least_squares (TRF).

        Residual vector stacks:
          - Fit residuals per (g,i,x): sqrt(w_obs) * r, where r = f - y (or relative if enabled)
          - Optional smoothness residuals: sqrt(lambda_s) * sqrt(w_smooth) * d2f/dx2
          - Optional similarity residuals: sqrt(lambda_sim) * (theta_g - theta_bar)

        Bounds are respected via TRF; constraints (monotonic/order/denominator) are NOT enforced here,
        matching the "unconstrained LS" demo path you saw earlier.
        """
        ...
        def unpack(tflat):
            return self._theta_flat_to_dict(tflat)

        def residual_vec(tflat: np.ndarray) -> np.ndarray:
            thetas = unpack(tflat)
            res = []
            ...
            for g in self.groups:
                ...
                for i_val in self.i_values:
                    ...
                    if xg.size == 0:
                        continue
                    fval = self.model.f(xg, float(i_val), th)
                    r = fval - yg
                    if getattr(cfg, "relative_error", False):
                        den = np.maximum(np.abs(yg), float(getattr(cfg, "rel_eps", 1e-6)))
                        r = r / den
                    res.append(np.sqrt(wg) * r)
            ...
            if cfg.lambda_s and cfg.lambda_s > 0:
                for g in self.groups:
                    ...
                    for i_val in self.i_values:
                        ...
                        res.append(np.sqrt(cfg.lambda_s) * np.sqrt(w) * d2)

            # Similarity residuals
            if cfg.lambda_sim and cfg.lambda_sim > 0:
                Theta = np.stack([thetas[float(g)] for g in self.groups], axis=0)
                theta_bar = np.mean(Theta, axis=0)
                for g_idx, g in enumerate(self.groups):
                    res.append(np.sqrt(cfg.lambda_sim) * (Theta[g_idx] - theta_bar))

            if len(res) == 0:
                return np.zeros_like(tflat)
            return np.concatenate(res).astype(float)

        lb = np.full_like(theta0_flat, cfg.lower_bound, dtype=float)
        ub = np.full_like(theta0_flat, cfg.upper_bound, dtype=float)
        ...
        lsq = least_squares(
            ...
            max_nfev=max(10000, 10*cfg.maxiter),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--maxiter", type=int, default=2000, help="Max iterations for primary solver.")
    ...
    p.add_argument("--method-primary", type=str, default="trust-constr", choices=["trust-constr", "SLSQP"], help="Primary optimizer for 'minimize' solver path.")
    p.add_argument("--method-fallback", type=str, default="SLSQP", choices=["SLSQP", "none"], help="Fallback optimizer (use 'none' to disable).")
    p.add_argument("--restarts", type=int, default=1, help="Number of multi-start restarts.")
    p.add_argument("--balance-i", dest="balance_i", action="store_true", default=False,
                   help="Scale contributions across i based on the range of y_i to balance magnitudes.")
    # Relative error options
    p.add_argument("--relative-error", dest="relative_error", action="store_true", default=False,
                   help="Use relative residuals r_rel=(f-y)/max(|y|, rel_eps) in the fit term.")
    p.add_argument("--rel-eps", type=float, default=1e-6, dest="rel_eps",
                   help="Denominator floor (rel_eps) for relative residuals.")
    # Solver choice
    p.add_argument("--solver", type=str, choices=["minimize", "least_squares"], default="minimize",
                   help="Optimization path: 'minimize' (SLSQP/trust-constr) or 'least_squares' (TRF).")
    return p.parse_args()
...
def main():
    ...
    cfg = SolverConfig(
        ...
        relative_error=bool(getattr(args, "relative_error", False)),
        rel_eps=float(getattr(args, "rel_eps", 1e-6))
    )
    ...
    for restart_idx in range(num_restarts):
        ...
        cfg_k = SolverConfig(
            ...
            relative_error=getattr(cfg, "relative_error", False),
            rel_eps=getattr(cfg, "rel_eps", 1e-6),
        )
        fitter_k = FitGlobal(df=df, model=model, grid_points=int(args.grid_points), config=cfg_k, outdir=outdir)
        theta_hat_k, info_k = (fitter_k.solve_least_squares() if (getattr(args, 'solver', 'minimize')=='least_squares') else fitter_k.solve())
    ...
    if getattr(args, "solver", "minimize") == "least_squares":
        if not info.get("success", False):
            print("Least-squares solver failure. See checks.json for details.", file=sys.stderr)
            sys.exit(2)
        else:
            print("Least-squares fitting completed successfully.")
            sys.exit(0)
    else:
        violated = (
