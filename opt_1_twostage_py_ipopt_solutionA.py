...
class CasadiModel:
    ...
    def f(self, x_vec: np.ndarray, i_val: float, theta_vec: np.ndarray) -> np.ndarray:
        ...
        ii = float(i_val)
        th = ca.DM(theta_vec)
        out = [float(self.f_fun(float(xv), ii, th)) for xv in np.ravel(x_vec)]
        return np.asarray(out, dtype=float)

    def dfdx(self, x_vec: np.ndarray, i_val: float, theta_vec: np.ndarray) -> np.ndarray:
        ii = float(i_val); th = ca.DM(theta_vec)
        return np.asarray([float(self.dfdx_fun(float(xv), ii, th)) for xv in np.ravel(x_vec)], dtype=float)

    def d2fdx2(self, x_vec: np.ndarray, i_val: float, theta_vec: np.ndarray) -> np.ndarray:
        ii = float(i_val); th = ca.DM(theta_vec)
        return np.asarray([float(self.d2fdx2_fun(float(xv), ii, th)) for xv in np.ravel(x_vec)], dtype=float)

    def denom(self, x_vec: np.ndarray, i_val: float, theta_vec: np.ndarray) -> Optional[np.ndarray]:
        if self.denom_fun is None:
            return None
        ii = float(i_val); th = ca.DM(theta_vec)
        return np.asarray([float(self.denom_fun(float(xv), ii, th)) for xv in np.ravel(x_vec)], dtype=float)
...
class FitGlobalCasadi:
        ...
        for idx_g, g in enumerate(self.groups):
            ...
            for i_val in self.i_values:
                ...
                for k in range(xs.size):
                    ...
                    pred = self.model.f_fun(xk, float(i_val), th)
                    r = pred - yk
            ...
            for idx_g, g in enumerate(self.groups):
                ...
                for i_val in self.i_values:
                    ...
                    for k in range(xs.size):
                        ...
                        d2 = self.model.d2fdx2_fun(xk, float(i_val), th)
                        if cfg.balance_i:
                            yi = self.df[(self.df["i"]==i_val) & (~self.df["y"].isna())]["y"].to_numpy()
                            rng = float(np.max(yi) - np.min(yi) + 1e-12)
                            wk = wk / rng
    ...
    def _update_order_by_median(self, theta: ca.SX) -> None:
        ...
        for i_val in self.i_values:
            ...
            for idx_g, g in enumerate(self.groups):
                th = theta[idx_g*P:(idx_g+1)*P]
                ys = [float(self.model.f_fun(float(xk), float(i_val), th)) for xk in xs]
    ...
    def _constraints_expr(self, theta: ca.SX) -> ca.SX:
        ...
        for i_val in self.i_values:
            ...
            for idx_g, g in enumerate(self.groups):
                th = theta[idx_g*P:(idx_g+1)*P]
                for xk in xs:
                    d1 = self.model.dfdx_fun(float(xk), float(i_val), th)
        ...
        for i_val in self.i_values:
            ...
            for j in range(len(order)-1):
                ...
                for xk in xs:
                    y_hi = self.model.f_fun(float(xk), float(i_val), th_hi)
                    y_lo = self.model.f_fun(float(xk), float(i_val), th_lo)
        ...
        if self.model.denom_fun is not None:
            for i_val in self.i_values:
                ...
                for idx_g, g in enumerate(self.groups):
                    th = theta[idx_g*P:(idx_g+1)*P]
                    for xk in xs:
                        den = self.model.denom_fun(float(xk), float(i_val), th)
