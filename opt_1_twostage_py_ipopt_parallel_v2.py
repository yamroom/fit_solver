...
def main() -> None:
    ...
    if (args.workers <= 1 or args.orchestrated) and (ca is None):
    ...
    if args.workers > 1 and not args.orchestrated:
        ...
        def _run_one(seed):
            ...
            sub_out = os.path.join(args.outdir, f'_run_seed{seed}')
            ...
            argv = [sys.executable, os.path.abspath(__file__),
                    ...
                    '--auto-direction', 'true' if args.auto_direction else 'false',
                    '--order-mode', str(args.order_mode),
                    '--lambda-s', str(args.lambda_s), '--lambda-sim', str(args.lambda_sim),
                    '--eps-mono', str(args.eps_mono), '--eps-ord', str(args.eps_ord), '--eps-den', str(args.eps_den),
                    '--rel-eps', str(args.rel_eps)]
            if args.relative_error:
                argv.append('--relative-error')
            argv += ['--restarts','1','--maxiter', str(args.maxiter), '--seed', str(seed)]
            if args.verbose:
                argv.append('--verbose')
            if getattr(args, 'ipopt_tol', None) is not None:
                argv += ['--ipopt.tol', str(args.ipopt_tol)]
            if getattr(args, 'ipopt_acceptable_tol', None) is not None:
                argv += ['--ipopt.acceptable_tol', str(args.ipopt_acceptable_tol)]
            if getattr(args, 'ipopt_hessian_approximation', None):
                argv += ['--ipopt.hessian_approximation', str(args.ipopt_hessian_approximation)]
            if getattr(args, 'ipopt_linear_solver', None):
                argv += ['--ipopt.linear_solver', str(args.ipopt_linear_solver)]
            argv += ['--plot-mode', str(getattr(args,'plot_mode','both'))]
            ...
            try:
                ...
                score = _score_from_checks_json(os.path.join(sub_out, 'checks.json'))
                return (seed, score, sub_out, cp.returncode)
            except Exception as e:
                return (seed, None, sub_out, 1)

        with ProcessPoolExecutor(max_workers=int(args.workers)) as ex:
            futs = [ex.submit(_run_one, s) for s in seeds]
