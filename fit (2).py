...
class SympyModel:
    ...
    dfdx_lam: callable
    d2fdx2_lam: callable
    df_dp_lam: callable              # returns shape (n_params, ...)
    ddfdx_dp_lam: callable           # returns shape (n_params, ...)
    d2fdx2_dp_lam: callable          # returns shape (n_params, ...)
    dden_dp_lam: Optional[callable]  # returns shape (n_params, ...), or None if denom==1
    denom_lam: Optional[callable]    # returns scalar/array denom, or None if denom==1
    ...
    def parse(expr_str: str) -> "SympyModel":
        ...
        # Derivatives wrt parameters (as matrices for efficient lambdify)
        df_dp = sp.Matrix([sp.diff(expr, p) for p in param_syms]) if param_syms else sp.Matrix([])
        dfdx = sp.diff(expr, x_sym)
        ddfdx_dp = sp.Matrix([sp.diff(dfdx, p) for p in param_syms]) if param_syms else sp.Matrix([])
        d2fdx2 = sp.diff(expr, x_sym, 2)
        d2fdx2_dp = sp.Matrix([sp.diff(d2fdx2, p) for p in param_syms]) if param_syms else sp.Matrix([])

        # Denominator derivatives wrt params (for c_den jacobian); None if denom == 1
        if denom_expr == 1:
            dden_dp = None
        else:
            dden_dp = sp.Matrix([sp.diff(denom_expr, p) for p in param_syms]) if param_syms else sp.Matrix([])
        ...
        df_dp_lam = lambdify(args, df_dp, "numpy") if param_syms else (lambda *a: np.zeros((0,)))
        ddfdx_dp_lam = lambdify(args, ddfdx_dp, "numpy") if param_syms else (lambda *a: np.zeros((0,)))
        d2fdx2_dp_lam = lambdify(args, d2fdx2_dp, "numpy") if param_syms else (lambda *a: np.zeros((0,)))
        dden_dp_lam = lambdify(args, dden_dp, "numpy") if dden_dp is not None else None
        denom_lam = lambdify(args, denom_expr, "numpy") if dden_dp is not None else None
        ...
        return SympyModel(
            ...
            df_dp_lam=df_dp_lam,
            ddfdx_dp_lam=ddfdx_dp_lam,
            d2fdx2_dp_lam=d2fdx2_dp_lam,
            dden_dp_lam=dden_dp_lam,
            denom_lam=denom_lam
        )

    # ---------- Helper to normalize parameter-derivative shapes ----------
    def _stack_param_derivs(self, raw, x_len: int) -> np.ndarray:
        """
        Normalize lambdified parameter-derivative output into a (P, x_len) float array.

        SymPy+NumPy can return a heterogeneous structure:
        - params not in expression: scalar 0
        - params in expression: vector of length x_len
        - P==1: may return a single 1D vector (x_len,)
        This normalizes all cases by broadcasting scalars to length x_len and stacking rows.
        """
        ...
        if isinstance(raw, (list, tuple)):
            ...
            if arr.ndim == 1 and arr.size == x_len and P == 1:
                rows_raw = [arr]
            elif arr.ndim >= 2 and arr.shape[0] == P:
                rows_raw = [arr[k, ...] for k in range(P)]
            else:
                rows_raw = list(arr.ravel())

        # Ensure exactly P rows
        if len(rows_raw) < P:
            rows_raw = rows_raw + [0.0] * (P - len(rows_raw))
        elif len(rows_raw) > P:
            rows_raw = rows_raw[:P]
    ...
    def df_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> np.ndarray:
        raw = self.df_dp_lam(x, i_val, *theta)
        x_len = int(np.size(x))
        return self._stack_param_derivs(raw, x_len)

    def ddfdx_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> np.ndarray:
        raw = self.ddfdx_dp_lam(x, i_val, *theta)
        x_len = int(np.size(x))
        return self._stack_param_derivs(raw, x_len)

    def d2fdx2_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> np.ndarray:
        raw = self.d2fdx2_dp_lam(x, i_val, *theta)
        x_len = int(np.size(x))
        return self._stack_param_derivs(raw, x_len)
    ...
    def dden_dp(self, x: np.ndarray, i_val: float, theta: np.ndarray) -> Optional[np.ndarray]:
        if self.dden_dp_lam is None:
            return None
        raw = self.dden_dp_lam(x, i_val, *theta)
        x_len = int(np.size(x))
        return self._stack_param_derivs(raw, x_len)
...
class FitSingleI:
    ...
    def _make_linspace_inclusive(a: float, b: float, step: float) -> np.ndarray:
        ...
        n = max(2, int(math.floor((b - a) * 1.0 / step)) + 1)
    ...
    def _constraints_den(self, theta_flat: np.ndarray) -> Tuple[np.ndarray, sparse.csr_matrix]:
        ...
        """
        if self.model.denom_lam is None or self.model.dden_dp_lam is None:
