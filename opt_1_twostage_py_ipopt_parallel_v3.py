...
def main() -> None:
    ...
    if args.workers > 1 and not args.orchestrated:
        seeds = [int(args.seed) + k for k in range(int(max(1, args.restarts)))]
        ...
        def _run_one(seed):
            ...
            argv = [sys.executable, os.path.abspath(__file__),
                    ...
                    '--auto-direction', 'true' if str2bool(args.auto_direction) else 'false',
            ...
            if getattr(args, 'ipopt_acceptable_tol', None) is not None:
                argv += ['--ipopt.acceptable_tol', str(args.ipopt_acc_tol)]
            if getattr(args, 'ipopt_hessian_approximation', None):
                argv += ['--ipopt.hessian_approximation', str(args.ipopt_hess)]
