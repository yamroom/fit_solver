...
# -*- coding: utf-8 -*-
"""
fit.py — Global constrained curve fitting (shared θ per g across all i)
======================================================================

Improvements for difficult cases where one i fits well but another doesn't:
1) **Per‑i balancing** (`--per-i-weighting equal|none`): make each i contribute
   roughly equally to the loss so a dense i doesn't dominate others.
2) **Adaptive Huber delta per i** (`--huber-auto-delta true` with `--huber-delta-scale`):
   delta_i ≈ scale * MAD(y_i), stabilizing across i with different magnitudes.
3) **Multi‑start** (`--restarts`, `--restart-noise`): random perturbations around
   the robust LS initialization per g to avoid poor local minima.
4) **Staged (continuation) solve** (`--stages`): solve a short sequence of problems,
   from easier (stronger smoothing, looser margins) to target config; each stage
   warm‑starts the next. This often fixes cases where both i can be well‑fit but
   the optimizer gets stuck.

Hard constraints preserved:
  - Monotonicity per (i,g): s_i * d/dx f >= eps_mono
  - Non‑crossing per i: f_{g(j+1)} - f_{g(j)} >= eps_ord
  - Denominator safety: den^2 - eps_den^2 >= 0 (if function has a denominator)

Other features:
  - SymPy parsing and automatic derivatives; per‑parameter lambdify to avoid shape issues.
  - SLSQP fallback with **dense** constraint Jacobian to avoid shape mismatches.
  - All numeric evaluations wrapped in np.errstate to silence transient warnings.
"""
...
class SolverConfig:
    ...
    huber_auto_delta: bool = False
    huber_delta_scale: float = 1.0
    per_i_weighting: str = "equal"  # 'equal' or 'none'
    ...
    stages: int = 2  # number of continuation stages >=1
...
class FitGlobal:
    def __init__(
        ...
        self.obs_count_by_i: Dict[float, int] = {}
        for i_val in self.i_values:
            ...
            for g in self.groups:
                ...
                count_i += sub.shape[0]
            self.obs_count_by_i[float(i_val)] = int(count_i)

        # per‑i weights
        if self.config.per_i_weighting == "equal":
            mean_n = np.mean([max(c,1) for c in self.obs_count_by_i.values()])
            self.i_weight = {float(i): (mean_n / max(self.obs_count_by_i[float(i)], 1)) for i in self.i_values}
        else:
            self.i_weight = {float(i): 1.0 for i in self.i_values}

        # Huber delta per i
        if self.config.huber_auto_delta:
            self.huber_delta_i = {}
            for i_val in self.i_values:
                yvals = []
                for g in self.groups:
                    ...
                    yvals.append(obs["y"])
                ycat = np.concatenate(yvals) if len(yvals) else np.array([1.0])
                self.huber_delta_i[float(i_val)] = max(1e-6, self.config.huber_delta_scale * mad(ycat))
        else:
            self.huber_delta_i = {float(i): self.config.huber_delta for i in self.i_values}
        ...
        # initial θ_g using all i's data for each g
            ...
            if xg.size >= max(2, self.P):
                ...
                try:
                    res = least_squares(
                        ...
                        loss="huber", f_scale=1.0, max_nfev=800, verbose=0
                    )
    ...
    # Objective & grad with per‑i balancing and per‑i Huber delta
    def _objective_and_grad(self, theta_flat: np.ndarray) -> Tuple[float, np.ndarray, Dict[str, float]]:
        ...
        for idx_g, g in enumerate(self.groups):
            ...
            for i_val in self.i_values:
                ...
                if xg.size == 0: 
                    continue
                alpha_i = self.i_weight[float(i_val)]
                delta_i = self.huber_delta_i[float(i_val)]
                f = self.model.f(xg, float(i_val), th)
                r = f - yg
                v, dr = huber_value_and_grad(r, delta_i)
                L_fit += float(np.sum(alpha_i * wg * v))
                J = self.model.df_dp(xg, float(i_val), th)  # (P,N)
                grad_acc += J @ (alpha_i * wg * dr)
        ...
        for idx_g, g in enumerate(self.groups):
            ...
            for i_val in self.i_values:
                ...
                d2 = self.model.d2fdx2(xs, float(i_val), th)
    ...
    # Staged + multi-start solve
    def solve(self) -> Tuple[np.ndarray, Dict]:
        best = None
        best_info = None
        theta0_flat_base = self._theta_dict_to_flat(self.theta0)
        rng = np.random.default_rng(self.config.seed if self.config.seed is not None else 123)

        nstarts = max(1, int(self.config.restarts))
        for s in range(nstarts):
            if s == 0:
                x0 = theta0_flat_base.copy()
            else:
                noise = rng.normal(scale=self.config.restart_noise, size=theta0_flat_base.shape)
                x0 = np.clip(theta0_flat_base + noise, self.config.lower_bound, self.config.upper_bound)

            # staged continuation
            stages = max(1, int(self.config.stages))
            x_curr = x0
            info_last = None
            for k in range(stages):
                # interpolate config
                t = (k + 1) / stages
                old_eps_mono = self.config.eps_mono; old_eps_ord = self.config.eps_ord; old_lambda_s = self.config.lambda_s
                # start a bit easier: 1/3 margins and 3x smoothing at stage 1; move to full at last stage
                self.config.eps_mono = (0.33 + 0.67*t) * old_eps_mono
                self.config.eps_ord  = (0.33 + 0.67*t) * old_eps_ord
                self.config.lambda_s = (3.0 - 2.0*t) * old_lambda_s
                ...
                res = minimize(
                    ...
                    options={"maxiter": max(200, self.config.maxiter // stages), "verbose": 3 if self.config.verbose else 0},
                )
                if (not res.success) and (self.config.method_fallback is not None):
                    res2 = minimize(
                        ...
                        options={"maxiter": max(200, self.config.maxiter // stages), "ftol": 1e-9, "disp": self.config.verbose},
                    )
                    if res2.success:
                        res = res2
                x_curr = res.x
                ...
                # restore original config numbers for next interpolation
                self.config.eps_mono = old_eps_mono; self.config.eps_ord = old_eps_ord; self.config.lambda_s = old_lambda_s

            # collect
            cons_vals, _ = self.constraints_all(x_curr)
            _, _, diag = self._objective_and_grad(x_curr)
            info = {
                "success": bool(info_last.success),
                "status": int(getattr(info_last, "status", -1)),
                "message": str(getattr(info_last, "message", "")),
                "niter": int(getattr(info_last, "nit", -1)),
                "fun": float(getattr(info_last, "fun", np.nan)),
            ...
            feasible = (info["max_constraint_violation"] <= 1e-9) and info["success"]
            if best is None or (feasible and (info["fun"] < best_info["fun"])) or ((not best_info.get("success", False)) and info["success"]):
                best = x_curr.copy()
                best_info = info
    ...
    def export_plots(self, theta_flat: np.ndarray, outdir: str) -> None:
        ...
        for i_val in self.i_values:
            ...
            plt.title(f"i = {i_val} — fitted f(x) by g (shared θ; balanced)")
...
def parse_args() -> argparse.Namespace:
    ...
    # New knobs
    p.add_argument("--per-i-weighting", type=str, choices=["equal","none"], default="equal",
                   help="Balance contribution across i (default equal).")
    p.add_argument("--huber-auto-delta", type=str, default="true",
                   help="If true, use per-i delta ≈ scale * MAD(y_i); else use --huber-delta.")
    p.add_argument("--huber-delta", type=float, default=1.0, dest="huber_delta",
                   help="Huber delta when auto is false.")
    p.add_argument("--huber-delta-scale", type=float, default=1.0,
                   help="Scale factor for auto delta (delta_i = scale * MAD(y_i)).")
    p.add_argument("--restarts", type=int, default=5, help="Number of random restarts.")
    p.add_argument("--restart-noise", type=float, default=0.1, help="Std of Gaussian noise around LS init.")
    p.add_argument("--stages", type=int, default=2, help="Number of continuation stages (>=1).")
    return p.parse_args()
...
def main():
    ...
    cfg = SolverConfig(
        ...
        huber_auto_delta=(str(args.huber_auto_delta).lower() in ("true","1","yes","y")),
        huber_delta_scale=float(args.huber_delta_scale),
        per_i_weighting=str(args.per_i_weighting),
        ...
        restart_noise=float(args.restart_noise),
        stages=int(args.stages),
    )

    fitter = FitGlobal(df=df, model=model, grid_points=int(args.grid_points), config=cfg, outdir=outdir)
    theta_hat, info = fitter.solve()

    # Checks
    per_i_checks = fitter.compute_checks_per_i(theta_hat)
    checks = {
        "spec_version": "2025-08-28-shared-theta-balanced",
        "function": expr_str,
        ...
        "config": {
            ...
            "per_i_weighting": cfg.per_i_weighting,
            "huber_auto_delta": cfg.huber_auto_delta,
            "huber_delta_scale": cfg.huber_delta_scale,
            "restarts": cfg.restarts,
            "restart_noise": cfg.restart_noise,
            "stages": cfg.stages,
        }
    }
    ...
    if violated:
        ...
        print("Fitting completed successfully (shared θ per g; balanced/staged).")
        sys.exit(0)
